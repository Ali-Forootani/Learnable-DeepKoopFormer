...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 29 11:10:11 2025
...
Extended with:
  ...
  - Learnable Koopman family selector: --learnable_kind scalar|permode|mlp|lowrank16|...
...
class Koopformer_Informer_Learnable(nn.Module):
    ...
    def __init__(self, input_dim, seq_len, horizon,
        ...
        self.backbone = InformerSparse(input_dim,
                                       ...
                                       seq_len=seq_len,
                                       patch_len=patch_len)
        self.koop = make_learnable_koopman(d_model= d_model, latent_dim=d_model, ρ_max=ρ_max) \
            if False else make_learnable_koopman(koop_kind, d_model, ρ_max=ρ_max)
        self.fc   = nn.Linear(d_model, horizon * input_dim)
...
def main(file="bitstamp_windows.npy", epochs=200, seq_len=120,
    ...
    for patch_len in patch_lens:
        for horizon in horizons:
            ...
            print(f"== PatchTST – learnable Koopman ({learnable_kind}) ==")
            m_pl = Koopformer_PatchTST_Learnable(
                F, patch_len, horizon,
                patch_len=patch_len, d_model=96, ρ_max=0.99,
                koop_kind=learnable_kind
            )
            ...
            res_loss[f"Koop-PatchTST (learn:{learnable_kind})"] = {
            ...
            test_preds[f"Koop-PatchTST (learn:{learnable_kind})"] = test_pred_pl

            prefix_pl = save_dir / f"PatchTST_learn-{learnable_kind}_{patch_len}_{horizon}"
            save_results(train_pred_pl, y_train_np, f"Koop-PatchTST (learn:{learnable_kind})",
                         prefix_pl, metrics_file,
                         patch_len, horizon, set_name="Train")
            save_results(test_pred_pl, y_test_np, f"Koop-PatchTST (learn:{learnable_kind})",
                         prefix_pl, metrics_file,
                         patch_len, horizon, set_name="Test")
            ...
            print(f"== Autoformer – learnable Koopman ({learnable_kind}) ==")
            ...
            res_loss[f"Koop-Autoformer (learn:{learnable_kind})"] = {
            ...
            test_preds[f"Koop-Autoformer (learn:{learnable_kind})"] = test_pred_al

            prefix_al = save_dir / f"Autoformer_learn-{learnable_kind}_{patch_len}_{horizon}"
            save_results(train_pred_al, y_train_np, f"Koop-Autoformer (learn:{learnable_kind})",
                         prefix_al, metrics_file,
                         patch_len, horizon, set_name="Train")
            save_results(test_pred_al, y_test_np, f"Koop-Autoformer (learn:{learnable_kind})",
                         prefix_al, metrics_file,
                         patch_len, horizon, set_name="Test")
            ...
            print(f"== Informer – learnable Koopman ({learnable_kind}) ==")
            ...
            res_loss[f"Koop-Informer (learn:{learnable_kind})"] = {
            ...
            test_preds[f"Koop-Informer (learn:{learnable_kind})"] = test_pred_il

            prefix_il = save_dir / f"Informer_learn-{learnable_kind}_{patch_len}_{horizon}"
            save_results(train_pred_il, y_train_np, f"Koop-Informer (learn:{learnable_kind})",
                         prefix_il, metrics_file,
                         patch_len, horizon, set_name="Train")
            save_results(test_pred_il, y_test_np, f"Koop-Informer (learn:{learnable_kind})",
                         prefix_il, metrics_file,
                         patch_len, horizon, set_name="Test")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description=(
            ...
            "(constrained vs learnable vs unconstrained Koopman, "
            "with scalar/permode/mlp/lowrank learnable variants, HPC-ready)"
        )
    )
    ...
    p.add_argument("--learnable_kind", type=str, default="scalar",
                   help="Learnable Koopman variant: "
                        "scalar | permode | mlp | lowrank | lowrank16 | ...")
    args = p.parse_args()
