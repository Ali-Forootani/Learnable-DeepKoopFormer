...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 28 09:20:01 2025
...
Koopman variants:
  ...
  · LearnableKoopmanOperator         – bounded spectrum via learnable squashing
...
Extended with:
  ...
  - Per-backbone Lyapunov weights for:
      * constrained Koopman
      * learnable Koopman
...
Extended with constrained vs learnable vs unconstrained Koopman (ChatGPT)
"""
...
class LearnableKoopmanOperator(nn.Module):
    """
    ODO-style Koopman with *learnable* squashing of the diagonal entries,
    still bounded by ρ_max:

      Σ_i = ρ_max * sigmoid(α S_i + β),

    where α, β are trainable scalars.
    """
    def __init__(self, latent_dim: int, ρ_max: float = 0.99):
        super().__init__()
        self.U_raw = nn.Parameter(torch.randn(latent_dim, latent_dim))
...
class UnconstrainedKoopmanOperator(nn.Module):
    ...
    StrictStableKoopmanOperator / LearnableKoopmanOperator:
...
class Koopformer_PatchTST_Learnable(nn.Module):
    """
    PatchTST backbone + learnable-squash Koopman operator.
    """
    def __init__(self, input_dim, seq_len, horizon,
                 patch_len, d_model=64, ρ_max: float = 0.99):
        super().__init__()
        self.backbone = PatchTST_Backbone(input_dim, seq_len,
                                          patch_len=patch_len,
                                          d_model=d_model)
        self.koop = LearnableKoopmanOperator(d_model, ρ_max=ρ_max)
        self.fc   = nn.Linear(d_model, horizon * input_dim)
...
class Koopformer_Autoformer_Learnable(nn.Module):
    """
    Autoformer-style backbone + learnable-squash Koopman in horizon-space.
    """
    def __init__(self, input_dim, seq_len, horizon,
                 patch_len, d_model=16, ρ_max: float = 0.99):
        super().__init__()
        self.backbone = SimpleAutoformer(seq_len, horizon, input_dim,
                                         patch_len=patch_len,
                                         d_model=d_model)
        self.koop = LearnableKoopmanOperator(horizon, ρ_max=ρ_max)
        self.fc   = nn.Linear(horizon, horizon * input_dim)
...
class Koopformer_Informer_Learnable(nn.Module):
    """
    Informer-style backbone + learnable-squash Koopman.
    """
    def __init__(self, input_dim, seq_len, horizon,
                 patch_len, d_model=64, ρ_max: float = 0.99):
        super().__init__()
...
def get_koopman_spectrum(model: nn.Module, koop_attr: str = "koop"):
    ...
    Returns:
        ...
        - For StrictStableKoopmanOperator and LearnableKoopmanOperator:
    ...
    elif isinstance(obj, LearnableKoopmanOperator):
        with torch.no_grad():
            Σ = torch.sigmoid(obj.alpha * obj.S_raw + obj.beta) * obj.ρ_max
            return Σ.detach().cpu().numpy()
...
def main(file="bitstamp_windows.npy", epochs=200, seq_len=120,
    ...
    for patch_len in patch_lens:
        for horizon in horizons:
            ...
            print("== PatchTST – learnable Koopman ==")
            ...
            test_preds["Koop-PatchTST (learn.)"] = test_pred_pl

            prefix_pl = save_dir / f"PatchTST_learn_{patch_len}_{horizon}"
            save_results(train_pred_pl, y_train_np, "Koop-PatchTST (learn.)",
                         prefix_pl, metrics_file,
                         patch_len, horizon, set_name="Train")
            save_results(test_pred_pl, y_test_np, "Koop-PatchTST (learn.)",
                         prefix_pl, metrics_file,
                         patch_len, horizon, set_name="Test")
            ...
            print("== Autoformer – learnable Koopman ==")
            ...
            res_loss["Koop-Autoformer (learn.)"] = {
            ...
            test_preds["Koop-Autoformer (learn.)"] = test_pred_al

            prefix_al = save_dir / f"Autoformer_learn_{patch_len}_{horizon}"
            save_results(train_pred_al, y_train_np, "Koop-Autoformer (learn.)",
                         prefix_al, metrics_file,
                         patch_len, horizon, set_name="Train")
            save_results(test_pred_al, y_test_np, "Koop-Autoformer (learn.)",
                         prefix_al, metrics_file,
                         patch_len, horizon, set_name="Test")
            ...
            print("== Informer – learnable Koopman ==")
            ...
            test_preds["Koop-Informer (learn.)"] = test_pred_il

            prefix_il = save_dir / f"Informer_learn_{patch_len}_{horizon}"
            save_results(train_pred_il, y_train_np, "Koop-Informer (learn.)",
                         prefix_il, metrics_file,
                         patch_len, horizon, set_name="Train")
            save_results(test_pred_il, y_test_np, "Koop-Informer (learn.)",
                         prefix_il, metrics_file,
                         patch_len, horizon, set_name="Test")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Koopformer-PRO benchmark (constrained vs learnable vs unconstrained Koopman, HPC-ready)")
